...
from provision.dockerfile import ECOSYSTEM_TEMPLATES
...
_SCM_VERSION_ENV = "ENV SETUPTOOLS_SCM_PRETEND_VERSION=0.0.0+pyhunt\n"


def _fix_setuptools_scm(dockerfile: str, log: str) -> tuple[str, str] | None:
    """``setuptools-scm`` cannot find a version because ``.git`` is not there.

    This is not an exotic case: setuptools-scm derives the package version from
    git tags, provisioning copies the working tree without ``.git`` (it is
    large, and it is not needed to run the code), and the build then dies in
    metadata generation before a single dependency is installed::

        LookupError: Error getting the version from source `vcs`:
        setuptools-scm was unable to detect version for /app/target.

    A large share of modern Python packaging uses it, so any target that does
    failed to provision — and because provisioning failure means no container,
    it meant **no finding in that repository could ever be execution-proven**.

    The version number is a fiction and says so. Nothing downstream reads it:
    the image exists to run the target's code, not to publish it.
    """
    if "SETUPTOOLS_SCM_PRETEND_VERSION" in dockerfile:
        return None
    out = _insert_before_first_run(dockerfile, _SCM_VERSION_ENV)
    if out is None:
        return None
    return out, (
        "setuptools-scm could not derive a version without .git — pinned "
        "SETUPTOOLS_SCM_PRETEND_VERSION so metadata generation succeeds"
    )
...
REPAIR_RULES: list[tuple[str, tuple[str, ...], RepairFn]] = [
    ...
    ("setuptools_scm_no_git", (
        r"setuptools[-_]scm was unable to detect version",
        r"Error getting the version from source `vcs`",
        r"SETUPTOOLS_SCM_PRETEND_VERSION",
    ), _fix_setuptools_scm),
