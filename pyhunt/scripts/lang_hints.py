...
PYTHON — the same class, different APIs. Execution here is IMMEDIATE (at load
time, in this process), not deferred:
- No safe mode, the only fix is "don't use it here": pickle / cPickle /
  _pickle, marshal, dill, shelve, jsonpickle.decode, and the two that hide it
  behind a data-science name — `pandas.read_pickle` and `joblib.load`.
- Safe by default, UNSAFE when the default is overridden — check the call, not
  the library's reputation: `numpy.load(..., allow_pickle=True)`,
  `torch.load(..., weights_only=False)`.
- YAML: `yaml.load` is unsafe unless the loader is `SafeLoader`/`CSafeLoader`.
  ...
  code before PyYAML 5.4 (CVE-2020-14343). Not a finding: ruamel's
  `YAML(typ="safe").load(x)` — the receiver carries the safety, so the call
  reads like bare `yaml.load(` to a line-based scanner. Check what `yaml` is
  bound to before reporting one.
- Unsafe reflection (CWE-470): `importlib.import_module`, `__import__`,
  `pydoc.locate`, `globals()[name]`, `getattr(obj, name)` on a name from
  config, env, or a request.

BEFORE YOU CLEAR OR CONFIRM ONE, work these four:
1. WRAPPER INDIRECTION — the dangerous call is often one layer down, inside a
   `Serializer.load()` / `deserialize()` / `from_bytes()` helper. Grep the
   WRAPPER's call sites; the sink table only ever saw the wrapper's name.
2. LAUNDERING THROUGH CACHE / SESSION / QUEUE — data that looks internal at the
   sink may have entered the store from outside. A Redis/memcached value is
   only as trusted as the least-trusted writer to that key: find who WRITES the
   key that is later loaded, not just who reads it.
3. A SAFE LOADER THAT ISN'T — confirm the SafeLoader in use was not re-armed by
   an `add_constructor` / custom loader subclass that puts object construction
   back, and that a safe kwarg at the call site is not overridden by a config
   default elsewhere.
4. VERSION-DEFAULT DRIFT — several of these APIs flipped their default across
   releases (`torch.load` weights_only, `numpy.load` allow_pickle, PyYAML
   FullLoader). Read the project's PINNED version, not the latest docs: a call
   that is safe on current releases can be unsafe on the version installed.

Order matters for reflection: an allow-list checked AFTER the name is resolved
is too late — importing a module already ran its top-level code.""",
...
in prod = HIGH; missing log-retention setting on a dev account = LOW.""",
    ...
    "codegen": """\
This target GENERATES SOURCE CODE from a schema/IDL it does not control
(OpenAPI, JSON Schema, Avro, Protobuf, GraphQL, XSD, ...). You are reviewing
its escaping of untrusted free-text fields into the code it emits (CWE-94).

THE MECHANISM. A free-text schema field is written into a string literal or a
comment of the generated file without escaping. The attacker puts the
literal's OWN TERMINATOR in that text; the literal ends early and the rest of
their text becomes real code. Execution is DEFERRED — it happens when the
victim imports or runs the generated file, not inside the generator. Do not
dismiss a finding because "the generator itself never executes it"; the
generator's output IS the payload delivery.

    SOURCE:  text = schema_obj.get("description")   # attacker-authored
    SINK:    f'\"\"\"{text}\"\"\"'  /  '\"\"\"$x\"\"\"'.substitute(x=text)  /  {{ x }}
    TRIGGER: victim imports the generated module → injected code runs

FIND THE SOURCES. Grep the whole repo for every read of an untrusted free-text
schema field, then follow each one to where it lands in emitted text:

    .get("doc") / .get("description") / .get("comment") / .get("summary")
    .description / .comment / .summary / .doc / .title / .example
    leading_comments / trailing_comments / leading_detached_comments

FIND THE SINKS. The dangerous shapes are literal-builders, not eval/exec:
f-strings and `.format()` that wrap a value in `\"\"\"`/`'''`; `string.Template`
`.substitute()`/`.safe_substitute()`; Jinja `{{ x }}` on a template LINE that
is building a docstring or a `#` comment; plain `+`/join concatenation into a
triple-quoted literal; anything assembled then handed to `.write_text()` or
`open(..., "w")`.

SAFE, do not report: the value goes through `repr()`, `json.dumps()`, or is
built as an AST node (`ast.Constant` + `ast.unparse`). Those cannot be
terminated by their own content.

THE REAL WORK IS SANITIZER CORRECTNESS. An escape call on the path is NOT a
verdict — most real CVEs in this class had one. Check each of these
explicitly and say which one you checked:

1. DELIMITER MATCH — does the sanitizer escape the delimiter the sink
   actually uses? A helper that escapes `\"` is useless when the sink wraps in
   `\"\"\"`, and one that escapes `\"` but NOT `\\` lets a trailing backslash
   escape the escape (this is exactly GHSA-9x4c-63pf-525f's incomplete fix).
2. PARTIAL / EDGE-CASE TERMINATORS — is the escape defeated by `\\\"\"\"`, by
   an interleaved `\"\"\\\"`, or by a quote adjacent to the closing delimiter?
   For `#`-comment sinks, a bare `\\r` with no `\\n` ends a physical line for
   Python's tokenizer — does the sanitizer strip it?
3. COVERAGE, NOT EXISTENCE — a sanitizer applied correctly at ONE call site
   says nothing about the others. Enumerate EVERY call site of that field
   type repo-wide and check each individually. This is where these bugs
   actually live.
4. BESPOKE TEMPLATING — if output is built with `string.Template`, raw
   concatenation, or a hand-rolled loop rather than Jinja2 or an AST builder,
   trace one full source→sink path by hand. Check any `templates.py` /
   constants table for patterns like `'\"\"\"$x\"\"\"'` or `f'\"\"\"{x}\"\"\"'`
   feeding `.substitute()` a call or two away from the field read.
5. ESCAPE-THEN-REUSE — a value sanitized once (safely `repr()`'d into a
   docstring) but the ORIGINAL RAW variable reused later in the same render
   function in a different literal context: a decorator argument, a dict key,
   an import line, a type alias. The safe copy sitting next to the raw one is
   what makes this easy to miss.

PROOF. The convincing PoC is not "it executed" — it is that the injected text
BECAME CODE. Generate the file from a crafted schema, `ast.parse` it, and
show the marker parsed as a `Name`/keyword/statement rather than as string
text. That is safe to hand a maintainer and does not require running the
payload.

Report file + function + line, WHICH of the five checks failed, the exact
