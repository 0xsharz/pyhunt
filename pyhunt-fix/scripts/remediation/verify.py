...
:func:`pyhunt.sandbox.require` before calling in here; this module does not decide
...
**The child environment is built from an allowlist** — see :func:`_child_env`.
This module used to do ``dict(os.environ, PYTHONDONTWRITEBYTECODE="1")``, which
copied every host variable, ``ANTHROPIC_API_KEY`` and ``CLAUDE_CODE_OAUTH_TOKEN``
included, into a subprocess running model-generated test code against untrusted
target code. Both halves of that subprocess are attacker-influenced and either
can read ``os.environ``. That is fixed here rather than deferred, because a known
credential leak does not wait for a roadmap.
"""
...
from remediation.diffcapture import safe_relative_path
...
#
# Canonical implementation: `pyhunt/scripts/verify_patch.py`. It is duplicated
# here rather than imported because that module ships inside the `pyhunt` skill
# and this one inside `pyhunt-fix`; the two are separately installed, and a
# cross-skill import would be a runtime dependency between them. Duplication is
# the lesser evil for ~20 lines whose whole point is that they never grow.
#
# ALLOWLIST, never a denylist. A denylist is correct only for the credential
# names that existed when it was written: add a provider, rotate to a new
# variable name, run under a CI system that injects its own token, and it keeps
# reporting success while leaking. An allowlist drops the variable nobody
# anticipated, which is the only case that matters.
#
#   PATH      python3 / node are found through it; without it nothing starts
#   LANG      \
#   LC_ALL     > text encoding; the POSIX default breaks tests asserting on
#   LC_CTYPE  /  non-ASCII output, which then fails for the wrong reason
#   TZ        a test that formats timestamps otherwise depends on the host zone
#
# Deliberately absent: NODE_OPTIONS (can preload arbitrary JS), PYTHONSTARTUP,
# PYTHONPATH (set below, never inherited), HOME and TMPDIR (replaced), and every
# variable this list does not name.
_ENV_ALLOWLIST: tuple[str, ...] = ("PATH", "LANG", "LC_ALL", "LC_CTYPE", "TZ")

_DEFAULT_PATH = "/usr/local/bin:/usr/bin:/bin"

# A tripwire, not the filter. If someone widens the allowlist above with a
# credential-shaped name, this fails at import instead of shipping.
_CREDENTIAL_MARKERS = ("KEY", "TOKEN", "SECRET", "PASSWORD", "PASSWD",
                       "CREDENTIAL", "AUTH", "SESSION", "COOKIE", "PRIVATE")

for _name in _ENV_ALLOWLIST:
    if not re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", _name):
        raise RuntimeError(f"verify: {_name!r} is not a valid env var name")
    if any(marker in _name.upper() for marker in _CREDENTIAL_MARKERS):
        raise RuntimeError(
            f"verify: {_name!r} was added to the environment allowlist, but its "
            "name is credential-shaped. This child runs model-generated test "
            "code against untrusted target code; nothing that may carry a "
            "secret crosses into it.")
del _name


def _child_env(workspace: Path, home: Path) -> dict[str, str]:
    """The environment for the test child, built from :data:`_ENV_ALLOWLIST`.

    `PYTHONPATH` is SET to the workspace, never appended to the host's: the
    workspace root has to be importable because a generated test usually lands in
    `tests/` and imports the package from the repo root (Python puts the
    *script's* directory on `sys.path`, not the cwd, so `python3 tests/test_x.py`
    cannot see `app/` — and the resulting ModuleNotFoundError is indistinguishable
    from a genuinely missing dependency, so every test would come back
    `not_attempted`). Inheriting the host's PYTHONPATH on top of that would let
    whatever the operator happens to have on it shadow the target's own modules.

    `HOME` and `TMPDIR` point at a scratch directory the caller owns, so a test
    cannot write into the operator's home directory or reach
    `~/.aws/credentials` by way of `~`.
    """
    src = os.environ
    env = {name: src[name] for name in _ENV_ALLOWLIST
           if src.get(name) and "\x00" not in src[name]}
...
def _run_once(runner: TestRunner, workspace: Path, rel: str,
    ...
    # An allowlisted environment (see _child_env) and a scratch HOME that is
    # destroyed with the run. NOT `dict(os.environ, ...)`: this child is
    # model-generated test code running against untrusted target code, and the
    # host environment carries the operator's API keys.
    home = Path(tempfile.mkdtemp(prefix="pyhunt-verify-home-"))
    env = _child_env(workspace, home)
    try:
        try:
        ...
        try:
            ...
            try:
                ...
                out = ""
            return ERROR, f"{out or ''}\n[pyhunt] timed out after {timeout}s"

        out = out or ""
        if proc.returncode == 0:
            return PASS, out
        if proc.returncode == 127 or any(m in out
                                         for m in runner.unmet + _UNMET_GENERIC):
...
def verify_patch(workspace: Path, *, test_path: str | None, test_source: str,
    ...
    Executes target code — the caller must have cleared :func:`pyhunt.sandbox.require`.
